...
VEHICLESETTINGS = {
    AIRBORNE = {
        AH1Z = {
            ...
            Components = {
                Spotlight_1 = {
                    enabled = true,  --是否启用这个光源,true为真
                    description = 'Gunner Searchlight',  --炮手探照灯  
                    type = 2, -- Pointlight [1] ; Spotlight [2]
                    weaponmounted = true,
                    shape = 1, --1 0为圆锥体 1为平顶锥体 2为长方体,
                    frustumFov = 35.6789,  --光锥的视觉角度（Field of View），改大会让手电范围更开阔。
                    frustumAspect = 1.0, --frustumAspect‌ 通常指 ‌视锥体（Frustum）的宽高比
                    castShadowsMinLevel = 0,
                    castShadowsEnable = true,  --是否投射阴影
                    intensity = 5 * SPOTLIGHTMULTIPLICATOR,
                    radius = 200,  --照射半径
                    color = Vec3(1, 0.9, 0.8),  --暖白色
                    transform = {
                        left = Vec3(-1, 0, 0),
                        up = Vec3(0, 1, 0),
                        forward = Vec3(0, 0, 1),
                ...
                Pointlight_1 = {  --用于模拟探照灯附近区域的“泛光”效果
                    enabled = true,  --是否启用这个光源,true为真
